...
def create_trip_page_template(page_name, title="新页面"):
    ...
    # 然后将content保存到trips/templates/trips/trip2.html
    """
...
<body>
    ...
    <script>
        ...
        document.getElementById('comment-form').addEventListener('submit', function(e) {{
            ...
            fetch(`/trips/${{pageName}}/add_comment/`, {{
                method: 'POST',
                body: formData
            }})
            .then(response => response.json())
        ...
        function deleteComment(commentId) {{
            if (confirm('确定要删除这条评论吗？')) {{
                fetch(`/trips/${{pageName}}/delete_comment/${{commentId}}/`, {{
                    method: 'POST',
                    headers: {{
                        'X-CSRFToken': document.querySelector('[name=csrfmiddlewaretoken]').value
                    }}
        ...
        window.addEventListener('load', function() {{
            fetch(`/trips/${{pageName}}/views_likes/`)
            .then(response => response.json())
            .then(data => {{
